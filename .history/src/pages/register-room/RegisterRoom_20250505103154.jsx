...
import { getAllRooms } from "../../api/index"; // Import the API function
...
const RegisterRoom = () => {
  ...
  useEffect(() => {
    const fetchRooms = async () => {
      try {
        const data = await getAllRooms(); // Call the API function
        setRooms(data); // Store the fetched data in state
      } catch (err) {
        ...
        <div className={`flex-grow p-4 bg-white transition-all duration-300 ${isSidebarOpen ? "lg:pl-80" : "pl-4"}`}>
          ...
          {!loading && !error && step === 1 && (
            <RoomList
              ...
              rooms={rooms} // Pass the fetched rooms to RoomList
            />
          )}
          {step === 2 && (
            <DateTimePicker
